...
public class MenuService {
    ...
    public void mainMenu() {
        do {
            try {
                ...
                System.out.println(e.getExceptionInfo().getDateTime());
                System.out.println("----------------------------------------------------------\n\n");
            }
        } while (true);
    }
    ...
    private void displayCarsByChosenAttribute() {
        CAR_SERVICE
                .sortCarsByArgument(USER_DATA_SERVICE.getSortType(), USER_DATA_SERVICE.isOrderDescending())
                .forEach(System.out::println);
    }

    private void displayCarsWithGreaterMileageThanGiven() {
        CAR_SERVICE
                .sortCarsWithGreaterMileage(USER_DATA_SERVICE.getMileage())
                .forEach(System.out::println);
    }
    ...
    private void displayCarsWithParticularComponent() {
        CAR_SERVICE
                .getCarsWithParticularComponent()
                .forEach((key, value) -> System.out.println("\n" + key + " " + value + "\n"));
    }

    private void displayCarsFromGivenPriceRange() {
        CAR_SERVICE
                .getCarsFromGivenPriceRange(USER_DATA_SERVICE.getFirstPrice(), USER_DATA_SERVICE.getSecondPrice())
                .forEach(System.out::println);
    }
}
